...
export default class RbmContentComponent extends React.Component {
  renderTopHeader() {
    const {
  ...
  renderBrandWiseDetailing() {
    ...
    renderClmAggregatedHeader() {
      const { selectedBo, selectedAbm, selectedGroupAbm } = this.props;
      return (
        <View style={styles.tabletSales}>
          <Text style={styles.brandWiseDetailingHeading}>CLM Aggregated Efforts</Text>
          <View style={styles.rightButtons}>
            <TouchableOpacity
              style={styles.boButton}
              onPress={() => {
            ...
            >
              <Text style={styles.specialityButtonText}>{selectedBo ? selectedBo.name : 'All BO Efforts'}</Text>
              <Image style={styles.specialityButtonIcon} source={Images.DropDown} />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={styles.boButton}
              onPress={() => {
                if (this.AbmActionSheet) {
                  this.AbmActionSheet.show();
                }
              }}
            >
              <Text style={styles.specialityButtonText}>{selectedAbm ? selectedAbm.name : 'All ABM Efforts'}</Text>
              <Image style={styles.specialityButtonIcon} source={Images.DropDown} />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={styles.boButton}
              onPress={() => {
                if (this.AbmGroupActionSheet) {
                  this.AbmGroupActionSheet.show();
                }
              }}
            >
              <Text style={styles.specialityButtonText}>{selectedGroupAbm ? selectedGroupAbm.name : 'All ABM Group Efforts'}</Text>
              <Image style={styles.specialityButtonIcon} source={Images.DropDown} />
            </TouchableOpacity>
          </View>
        </View>
    ...
    renderAbmActionSheet = () => {
      const { setSelectedAbm, abms } = this.props;
      if (abms) {
        return (
          <ActionSheet
            ref={(o) => this.AbmActionSheet = o}
            title="Choose ABM"
            options={abms.map((spec) => spec.name)}
            onPress={(index) => {
              setSelectedAbm(abms[index]);
            }}
          />
        );
      }
    ...
    renderAbmGroupActionSheet = () => {
      const { setSelectedGroupAbm, abms } = this.props;
      if (abms) {
        return (
          <ActionSheet
            ref={(o) => this.AbmGroupActionSheet = o}
            title="Choose ABM Group"
            options={abms.map((spec) => spec.name)}
            onPress={(index) => {
              setSelectedGroupAbm(abms[index]);
            }}
          />
        );
      }
    ...
    renderAggregatedDetailingData() {
      const { detailingAggregatedEffort } = this.props;
      if (detailingAggregatedEffort) {
        return (
          <View style={styles.tabletSales}>
    ...
    renderAggregatedTotalCalls() {
      const { e_detailing_calls, v_detailing_calls, physical_detailing_calls } = this.props.detailingAggregatedEffort;
      return (
        <View style={styles.totalCallsContainer}>
          <Text style={styles.subHeading}>Total Calls</Text>
          <ChartView
    ...
    renderAggregatedAverageTimeSpent() {
      const { avg_edetail_time, avg_virtual_detail_time } = this.props.detailingAggregatedEffort;
      return (
        <View style={styles.avgTimeSpentContainer}>
          <Text style={styles.subHeading}>Avg. Time Spent</Text>
          <View style={styles.avgTimeSpentView}>
    ...
    renderAggregatedDetailingCompliance() {
      const { detailing_compliance, isBiologics } = this.props.detailingAggregatedEffort;
      if (isBiologics) {
        return (
          <View style={styles.detailingComplianceContainer}>
            <Text style={styles.subHeading}>Detailing Compliance</Text>
    ...
    renderAggregatedBrandWiseDetailing() {
      const { detailingAggregatedBrandEffort } = this.props;
      if (detailingAggregatedBrandEffort && detailingAggregatedBrandEffort.brands) {
        return (
          <ChartView
            originWhitelist={['']}
            style={styles.brandWiseDetailingChart}
            config={getBrandWiseDetailingChartUtil(detailingAggregatedBrandEffort ? detailingAggregatedBrandEffort.brands : [])}
            options={getChartConfig()}
          />
        );
      }
      ...
      return (
        ...
        >
          ...
          <ScrollView contentContainerStyle={styles.container}>
            ...
            {this.renderAbmActionSheet()}
            {this.renderAbmGroupActionSheet()}
          </ScrollView>
