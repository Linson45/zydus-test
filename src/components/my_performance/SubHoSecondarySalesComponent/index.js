...
export default class SubHoSecondarySalesComponent extends React.Component {
  renderTab1List() {
    const { subHoZoneWiseSales, onRegionItemPress } = this.props;
    if (!Array.isArray(subHoZoneWiseSales)) {
      return null;
    }
    return subHoZoneWiseSales.map((item, index) => {
      const {
  ...
  render() {
    const {
      loading, date_range, subHoZoneWiseSales, brandWiseSales
    } = this.props;
    ...
    return (
      ...
      >
        <Container>
          ...
          >
            <View tabLabel="Zone Wise Sales">
              <ScrollView ref={(scrollView) => this._scrollView = scrollView}>
                {subHoZoneWiseSales ? this.renderTab1List() : null}
              </ScrollView>
            </View>
