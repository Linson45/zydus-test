...
import {incompleteListData} from '../../../actions'
...
class RXDocListComponent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      searchQuery: '',
    ...
    this.approvedListShow = this.approvedListShow.bind(this);
    this.rejectedListShow = this.rejectedListShow.bind(this);
    this.pendingListShow = this.pendingListShow.bind(this);
  }
  componentDidMount() {
  ...
  async goToUploadVerficationScreen(docData, status) {
    const userDetails = this.props.navigation.state.params.user;
  if (status==-1) {
    const { loading, connected, data, data_1, rxTab } = this.props;
    const { user } = this.props.navigation.state.params;
    this.props.navigation.navigate('IncompleteRxEdit', {
      docData,
      userDetails,
      user,
      data,
              ...
              <View style={{ flexDirection: 'column' }}>
                <View style={{ flexDirection: 'row', marginTop: 10 }}>
                  <View style={{ flex: 1, flexDirection: 'column' }}>
                    ...
                    <Text style={styles.doc_item}>{spec_desc}</Text>
                    <Text style={styles.doc_item}>{visit_category}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
  ...
  renderRXTrackerList(data) {
    ...
    // if(dataOnline!=null){
    // var offlineDatas = []
    // this.props.dataFromOffline.map(list=>{
    //   offlineDatas.push(list)
    // })
    // var data = dataOnline.concat(offlineDatas)
    if (data != null) {
      const listToDisplay = data;
      ...
      return dataFiltered.map((item, index) => {
        ...
        return (
          <ListItem
            key={index}
            bordered
            onPress={() => this.goToUploadVerficationScreen(item, status)}
            style={{
              marginLeft: 0,
            }}>
            <Body>
                    ...
                    {status === '1' || status === '0' ?
                      <Text
                        ...
                        {status_rbm === '1'
                          ...
                          : status_rbm === '2'
                            ? 'REJECTED by RBM'
                            : status == '0' ? 'PENDING by ABM' : !status === -1 ? 'PENDING by RBM' : null}
                      </Text> : null
                    }
                    <Text
                      style={{
                        ...
                        color:
                          status === '1'
                            ...
                            : status === '2'
                              ? ColorStyles.red
                              : status === -1 ? 'blue' : ColorStyles.gray,
                      }}>
                      {status_rbm == '0' ? approvedOrRejected : status=== -1 ? 'INCOMPLETE':null}
                    </Text>
                  </View>
                </View>
              </View>
    ...
    //       <View style={{ flex: 1, flexDirection: 'column' }}>
    //         <Text style={styles.docTitle}>B INDIRA</Text>
    //       </View>
    //       <View style={{ flex: 1, flexDirection: 'column' }}>
    //         <Text style={styles.doc_item}>Corza</Text>
    //         <Text style={styles.doc_item}>Pantodac 40</Text>
    //       </View>
    //       <View style={{ flex: 1 }}>
    //         <Text style={styles.doc_item}>23-01-2021</Text>
    //       </View>
    //       <View style={{ flex: 1 }}>
    //         <Text
    //           style={{
  ...
  rxApprovedListShow() {
    ...
    this.setState({ rxIncompletedList: false });
    ;
  }
  rxRejectedListShow() {
    this.setState({ rxSortChangeStarted: false });
  ...
  rxIncompletedListShow() {
    ...
    this.setState({ rxPendingList: false });
    this.setState(prevState => ({
      rxIncompletedList: !prevState.rxIncompletedList
    }))
  }
  renderHeader() {
    return (
  ...
  renderStatus(data, data_1) {
    if (data_1 != null) {
      ...
      const incomplete = data_1.filter(item => item.status === -1);
      if (this.state.isTab) {
        return (
          <View style={{ flexDirection: 'row' }}>
            <View style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              <TouchableOpacity
                style={{
                  ...
                  borderColor: 'yellow',
                  // color: '#808080',
                  backgroundColor: this.state.rxIncompletedList
                    ? 'yellow'
                    : 'white',
                  padding: 5,
                  fontSize: 20,
                ...
                onPress={() => this.rxIncompletedListShow()}>
                <Text
                  style={{
                    fontSize: 20,
                    textAlign: 'center',
                    // color: this.state.rxIncompletedList ? 'black' : 'black',
                    color: 'black',
                  }}>{`${'INCOMPLETE' + ' ' + '('}${incomplete.length})`}</Text>
              </TouchableOpacity>
            </View>
          </View>
        );
      ...
      if (data != null) {
        ...
        console.log('Aproved,Rejected,pending', approved, rejected, pending)
        if (!this.state.isTab) {
          return (
            <View style={{ flexDirection: 'row' }}>
              <View style={{ flex: 1 }}>
  ...
  renderTabs(docList, user, rxData, rxTab) {
        ...
        <ScrollableTabView
          ...
          <View tabLabel={rxTracker} style={{ flex: 1 }}>
            ...
            <TouchableOpacity
              activeOpacity={0.7}
              onPress={() =>
                this.props.navigation.navigate('RxUpload', {
                  user,
                  bo,
                  rxData,
                  docList,
    ...
    return (
      <ParentView
        ...
        <Container>
          ...
          {this.renderTabs(data, user, data_1, rxTab)}
        </Container>
      </ParentView>
    );
  }
}
export default withNavigation(RXDocListComponent);
