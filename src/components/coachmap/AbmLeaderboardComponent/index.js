...
export default class AbmLeaderboardComponent extends React.Component {
  renderHeader() {
  ...
  renderCollapse() {
    ...
    if (data && Array.isArray(data)) {
      return data.map(({
        filed_by_user_type, filed_by_rep_name, files
      }, index) => (
        <Collapse key={index}>
          <CollapseHeader>
            <Card>
              <CardItem style={styles.collapseHeader}>
                <View>
                  ...
                  <Text style={styles.collapseHeaderSub}>{filed_by_user_type}</Text>
                </View>
          ...
          <CollapseBody style={styles.collapseBody}>
            <ScrollView>
              {Array.isArray(files) ? files.map((file, index) => (
                <TouchableOpacity key={index} onPress={() => gotoDetails(file)}>
