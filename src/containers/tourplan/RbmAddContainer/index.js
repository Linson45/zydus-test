...
import RbmAddComponent from '../../../components/tourplan/RbmAddComponent';

class TourplanRbmAddContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      comment: '',
  ...
  async firstLoad() {
    ...
    const {
      date, route, btp_code, bo_names, bo_codes, rbm_comment
    } = item;
    await this.setState({
      date,
      comment: rbm_comment || '',
  ...
  gotoAddBo() {
    const { user, item } = this.props.navigation.state.params;
    const { bo_codes, date } = this.state;
    this.props.navigation.navigate('TourplanSelectBo', {
      user,
      item,
      bo_codes,
      date,
